...
**URL**: `https://win-house-group.github.io/win-house-brand-assets/Win-House-Group-bk.png`
...
**URL**: `https://win-house-group.github.io/win-house-brand-assets/Win-House-Group-wt.png`
...
| Hong Kong 香港 | 文浩國際(香港)有限公司 | WIN-HOUSE INTERNATIONAL (H.K.) LTD. |
...
| GitHub Repo | `https://github.com/win-house-group/win-house-brand-assets` |
...
- 主要 URL 一律使用 `https://win-house-group.github.io/win-house-brand-assets/...`,預設不加任何參數。
...
- Canonical URLs are `https://win-house-group.github.io/win-house-brand-assets/...` with **no** query parameter by default.
...
- 主要 URL 一律使用 `https://win-house-group.github.io/win-house-brand-assets/...`,默认不加任何参数。
- 覆盖同名文件后,若用户浏览器仍显示旧图,仅需在该次链接后加 `?v=N`(例如 `?v=2`)破除**浏览器**缓存即可;源文件无需重命名。
- 不再使用 `purge.jsdelivr.net`(已停用)。

**Hosting note**: GitHub Pages, served from the `main` branch of `win-house-group/win-house-brand-assets`.
...
> https://win-house-group.github.io/win-house-brand-assets/design-system/
>
