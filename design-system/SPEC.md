...
| `--sp-3` | 13px | 12px | Card inner gap |
| `--sp-4` | 17px | 16px | Card padding |
| `--sp-5` | 25px | 24px | Section element gap |
| `--sp-6` | 34px | 32px | Large card padding |
| `--sp-7` | 50px | 48px | Section vertical |
| `--sp-8` | 67px | 64px | Major section |
| `--sp-9` | 92px | 88px | Hero / page padding |

> 注意:`.tag` 內距 (`4px 12px`) 與 `.stat-delta` 內距已硬寫為固定值,**不**綁定 `--sp-3`,故不隨上表變動。
...
### Dashboard Patterns · 儀表板規範 (v3.14)
...
WIN-HOUSE Group provides three logo variants. Choose by context while maintaining brand consistency.

### Variant 01 · Full Wordmark · 完整商標(Logo + 文字)

**URL**: `https://win-house-group.github.io/winhouse-brand-assets/Win-House-Group-bk.png`
...
**URL**: `https://win-house-group.github.io/winhouse-brand-assets/winhouse-logo-red.png`
...
### Variant 03 · White Wordmark · 反白商標(深色底專用) 🆕 v3.14.0

**URL**: `https://win-house-group.github.io/winhouse-brand-assets/Win-House-Group-wt.png`

White (reversed-out) version of the full wordmark, for use **only on dark backgrounds**. This replaces ad-hoc recoloring — never recolor the black logo by hand; use this asset.

**Recommended Use · 建議場景 · 建议场景**:
- Dark Hero (`#0A0A0A`), dark banners, event key visuals · 深色 Hero、深底 Banner、活動主視覺 · 深色 Hero、深底 Banner、活动主视觉
- Dark presentation covers, video intro/outro · 深色簡報封面、影片片頭/片尾 · 深色演示封面、视频片头/片尾
- Dark social posts, dark page footers · 深色社群貼文、深色頁尾 · 深色社群贴文、深色页尾

**Pairing rule · 配對規則**: light background → `Win-House-Group-bk.png`; dark background → `Win-House-Group-wt.png`. Never put the black logo on a dark surface or the white logo on a light surface.
...
| Asset | URL |
|---|---|
| WINGO Hi (greeting) | `https://win-house-group.github.io/winhouse-brand-assets/wingo/wingo-hi.png` |

**Current assets**: 1 · Greeting (Hi). New poses, expressions, and UI components will be added to the `wingo/` folder. URL pattern: `https://win-house-group.github.io/winhouse-brand-assets/wingo/[filename].png` — for a replaced same-name file, append an optional `?v=N` (e.g. `?v=2`) only to bust the **browser** cache; GitHub Pages itself serves the latest commit immediately.
...
All Win-House design resources are hosted on GitHub Pages (win-house-group.github.io), accessible from any AI or external system:

| Resource | URL |
|---|---|
| **This spec (Markdown)** | `https://win-house-group.github.io/winhouse-brand-assets/design-system/SPEC.md` |
| **Interactive HTML spec** | `https://win-house-group.github.io/winhouse-brand-assets/design-system/` |
| **Design Tokens CSS** | `https://win-house-group.github.io/winhouse-brand-assets/design-system/winhouse-tokens.css` |
| WINGO Hi greeting | `https://win-house-group.github.io/winhouse-brand-assets/wingo/wingo-hi.png` |
| Group Logo (full wordmark) | `https://win-house-group.github.io/winhouse-brand-assets/Win-House-Group-bk.png` |
| Group Logo (standalone mark) | `https://win-house-group.github.io/winhouse-brand-assets/winhouse-logo-red.png` |
| Logo source AI file | `https://win-house-group.github.io/winhouse-brand-assets/Win-House-Group-bk.ai` |
| WINGO image fallback (raw) | `https://win-house-group.github.io/winhouse-brand-assets/wingo/wingo-hi.png` |
| Brand README | `https://win-house-group.github.io/winhouse-brand-assets/README.md` |
| GitHub Repo | `https://github.com/win-house-group/winhouse-brand-assets` |

### 🔄 Asset Hosting & Cache · 資產主機與快取 · 资产主机与缓存

**[繁中]** v3.14.0 起,品牌資產主機已自 jsDelivr/GitHub raw 遷移至 **GitHub Pages**(`win-house-group.github.io`)。Pages 會直接服務最新 commit,**毋須** jsDelivr 的 7 天快取破解。流程簡化為:
- 主要 URL 一律使用 `https://win-house-group.github.io/winhouse-brand-assets/...`,預設不加任何參數。
- 覆蓋同名檔案後,若使用者瀏覽器仍顯示舊圖,僅需在該次連結後加 `?v=N`(例如 `?v=2`)破除**瀏覽器**快取即可;原始檔不需重新命名。
- 不再使用 `purge.jsdelivr.net`(已停用)。

**[English]** From v3.14.0, brand assets are hosted on **GitHub Pages** (`win-house-group.github.io`), migrated off jsDelivr/GitHub-raw. Pages serves the latest commit directly, so jsDelivr's 7-day cache-bust is **no longer needed**. Simplified flow:
- Canonical URLs are `https://win-house-group.github.io/winhouse-brand-assets/...` with **no** query parameter by default.
- If a replaced same-name file still shows stale in a user's browser, append `?v=N` (e.g. `?v=2`) to that link to bust the **browser** cache only — no need to rename the source file.
- `purge.jsdelivr.net` is **retired**.

**[简中]** v3.14.0 起,品牌资产主机已从 jsDelivr/GitHub raw 迁移至 **GitHub Pages**(`win-house-group.github.io`)。Pages 会直接服务最新 commit,**无需** jsDelivr 的 7 天缓存破解。流程简化为:
- 主要 URL 一律使用 `https://win-house-group.github.io/winhouse-brand-assets/...`,默认不加任何参数。
- 覆盖同名文件后,若用户浏览器仍显示旧图,仅需在该次链接后加 `?v=N`(例如 `?v=2`)破除**浏览器**缓存即可;源文件无需重命名。
- 不再使用 `purge.jsdelivr.net`(已停用)。

**Hosting note**: GitHub Pages, served from the `main` branch of `win-house-group/winhouse-brand-assets`.
...
> https://win-house-group.github.io/winhouse-brand-assets/design-system/
>
...
> - **For any HTML/web output, link the official tokens stylesheet first**: https://win-house-group.github.io/winhouse-brand-assets/design-system/winhouse-tokens.css
...
> - WINGO image: https://win-house-group.github.io/winhouse-brand-assets/wingo/wingo-hi.png
> - **Group Logo** (two variants):
>   - Full wordmark (logo + text): https://win-house-group.github.io/winhouse-brand-assets/Win-House-Group-bk.png
>   - Standalone mark: https://win-house-group.github.io/winhouse-brand-assets/winhouse-logo-red.png
